// Skills UI — works for both user skills (data-user-id) and project skills (data-project-id)
(function(){
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const ownerId = userId || projectId;
    const baseUrl = userId ? `/users/${userId}` : `/projects/${projectId}`;
    const searchUrl = userId ? '/users/skills/' : '/projects/skills/';
    ...
    container.addEventListener("click", async (e) => {
      if (e.target.classList.contains("remove-skill-btn")) {
        ...
        const res = await fetch(`${baseUrl}/skills/${skillId}/remove/`, {
          method: "POST",
    ...
    async function addSkillById(skillId) {
      const res = await fetch(`${baseUrl}/skills/add/`, {
        method: "POST",
        headers: {
